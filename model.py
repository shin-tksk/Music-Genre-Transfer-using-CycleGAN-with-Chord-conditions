...
class cyclegan(object):
    ...
    def _build_model(self):
        ...
        if self.model != 'base':
            ...
            self.DA_fake_sample_all = self.discriminator(self.fake_A_sample + self.gaussian_noise, self.options, reuse=True, name="discriminatorA_all")
            self.DB_real_all = self.discriminator(self.real_mixed + self.gaussian_noise, self.options, reuse=False,
                                                  name="discriminatorB_all")
            self.DB_fake_sample_all = self.discriminator(self.fake_B_sample + self.gaussian_noise, self.options, reuse=True, name="discriminatorB_all")
        # Generator loss
    ...
    def train(self, args):
        ...
        loss_dirpath = './Loss/{}/{}'.format(self.dataset_A_dir,'2022-05-08')
        os.makedirs(loss_dirpath,exist_ok = True)
        ...
        log_dir = './logs/{}2{}_{}_{}_{}'.format(self.dataset_A_dir, self.dataset_B_dir, '2022-05-08',
                                                 self.model, self.sigma_d)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                if self.model == 'base':
                    ...
                    sum_Gloss_AB += g_loss_a2b
                    sum_Gloss_BA += g_loss_b2a
                    sum_Closs += cycle_loss
                    sum_Dloss_A += da_loss
                    sum_Dloss_B += db_loss
            ...
            Gloss_AB = sum_Gloss_AB / batch_idxs
            Gloss_BA = sum_Gloss_BA / batch_idxs
            Closs = sum_Closs/batch_idxs
            Dloss_A = sum_Dloss_A / batch_idxs
            Dloss_B = sum_Dloss_B / batch_idxs

            Gloss_AB_list.append(Gloss_AB)
            Gloss_BA_list.append(Gloss_BA)
            Closs_list.append(Closs)
            Dloss_A_list.append(Dloss_A)
            Dloss_B_list.append(Dloss_B)

        print("G loss" , Gloss_AB + Gloss_BA + Closs)
        print("C loss" , Closs)
        print("D loss" , Dloss_A + Dloss_B)
        
        figG = plt.figure()
        plt.plot(epoch_list, Gloss_AB_list,label="AtoB")
        plt.plot(epoch_list, Gloss_BA_list,label="BtoA")
        plt.title("generator loss")
        plt.xlabel("epochs")
        plt.ylabel("loss")
        figG.savefig("./{}/Gloss.png".format(loss_dirpath))
        plt.close()

        figC = plt.figure()
        plt.plot(epoch_list, Closs_list)
        plt.title("cycle loss")
        plt.xlabel("epochs")
        plt.ylabel("loss")
        figC.savefig("./{}/Closs.png".format(loss_dirpath))
        plt.close()

        figD = plt.figure()
        plt.plot(epoch_list, Dloss_A_list, label="A")
        plt.plot(epoch_list, Dloss_B_list, label="B")
        plt.title("discriminator loss")
        plt.xlabel("epochs")
        plt.ylabel("loss")
        figD.savefig("./{}/Dloss.png".format(loss_dirpath))
        plt.close()
    ...
    def save(self, checkpoint_dir, step):
        ...
        #                                    self.sigma_d)
        model_dir = "{}2{}_{}_{}_{}".format(self.dataset_A_dir, self.dataset_B_dir, '2022-05-08', self.model,
                                            self.sigma_d)
    ...
    def test(self, args):
        ...
        test_dir_mid = os.path.join(args.test_dir, '{}2{}_2022-05-08_{}_{}/{}/mid'.format(self.dataset_A_dir,
                                                                                  self.dataset_B_dir,
                                                                                  self.model,
                                                                                  self.sigma_d,
        ...
        test_dir_npy = os.path.join(args.test_dir, '{}2{}_2022-05-08_{}_{}/{}/npy'.format(self.dataset_A_dir,
                                                                                  self.dataset_B_dir,
                                                                                  self.model,
                                                                                  self.sigma_d,
                                                                                  args.which_direction))
    ...
    def test_famous(self, args):
        ...
        TO = 'pop'
        MODE = 'test'
        os.makedirs('./datasets/npydata/transfer/{}_to_{}/{}'.format(GENRU,TO,MODE),exist_ok = True)
        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)
        data_famous = [f for f in os.listdir('./datasets/npydata/origin/{}/{}'.format(GENRU,MODE))]
        for i in range(len(data_famous)):
            try:
                song = np.load(os.path.join('./datasets/npydata/origin/{}/{}/{}'.format(GENRU,MODE,data_famous[i])))
                #song = np.load(os.path.join('./datasets/famous_song/origin/{}'.format(data_famous[i])))
                if self.load(args.checkpoint_dir):
                    print(" [*] Load SUCCESS")
                else:
                    print(" [!] Load failed...")
            ...
            #np.save(os.path.join('./datasets/famous_song/transfer/npy/', os.path.splitext(data_famous[i])[0] + '.npy'), transfer)
            np.save(os.path.join('./datasets/npydata/transfer/{}_to_{}/{}'.format(GENRU,TO,MODE), os.path.splitext(data_famous[i])[0] + '.npy'), transfer)
            #save_midis(transfer,os.path.join('./datasets/famous_song/transfer/midi', os.path.splitext(data_famous[i])[0] + '.mid'),100)
            zero1 = np.zeros([transfer.shape[0],192,24,1])
            zero2 = np.zeros([transfer.shape[0],192,48,1])
            melo,chord = np.split(transfer,[84],2)
            chord = np.concatenate((zero1,chord,zero2),axis=2)
            #save_midis(chord,os.path.join('./datasets/famous_song/transfer/chord', os.path.splitext(data_famous[i])[0] + '_chord.mid'),100)