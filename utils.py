...
def save_midis(bars, file_path, tempo=100.0):
    if bars.shape[2] > 84:
        bars = np.delete(bars,slice(84,112),2)
    ...
    #padded_bars = bars
    #pause = np.zeros((bars.shape[0], 192, 128, bars.shape[3]))
    images_with_pause = padded_bars
    images_with_pause = images_with_pause.reshape(-1, RESOLUTION*16, padded_bars.shape[2], padded_bars.shape[3])
    #print(images_with_pause.shape)
    ...
    #print(images_with_pause_list[1])
