...
def chord_detection(tonnetz,root):
    dis = distance.euclidean([0,0,0,0,0,0,],[100,100,100,100,100,100])
    name = "None"
    num = np.zeros(2)
    chord = np.zeros(13)
    for i in range(12):
...
def get_images(tonnetz,i):
    fig, ax = plt.subplots(figsize=(4,4))
    plt.plot(tonnetz[0],tonnetz[1],marker='.')
    ax.set_xticks([-2, -1, 0, 1, 2])
    ax.set_yticks([-2, -1, 0, 1, 2])
    ax.grid()
    c = patches.Circle( xy=(0,0), radius=1, fill=False) 
    ax.add_patch(c)
    fig.savefig(os.path.join(ROOT_PATH, 'fig/{}.png').format(i))
...
def sum_MIDI(ori):
    ori = ori.reshape(-1,84)
    ori = np.sum(ori,axis=0)
    ori = ori.reshape(7,12)
    ori = np.sum(ori,axis=0)
    return ori
