...
def set_piano_roll_to_instrument(piano_roll, instrument, velocity=100, tempo=100.0, beat_resolution=RESOLUTION):
    # Calculate time per pixel
    tpp = 60.0 / tempo / float(beat_resolution)
    threshold = 60.0 / tempo / RESOLUTION
    phrase_end_time = 60.0 / tempo * 4 * piano_roll.shape[0] * 4
    ...
    for note_num in range(128):
        ...
        #print(note_num,temp_start_time)
        temp_end_time = [i for i in end_time]
        #print(note_num,temp_end_time)
        ...
        #print(note_num, np.subtract(start_time, end_time))
        duration = [pair[1] - pair[0] for pair in zip(start_time, end_time)]
        #print('After checking, duration each note:', duration)
        #duration_bar = [(int)(x/2.4*48) for x in duration]
        #print(note_num,duration_bar)
        #print(len(duration))
...
def write_piano_rolls_to_midi(piano_rolls, program_nums=None, is_drum=None, filename='test.mid', velocity=100,
                              tempo=100.0, beat_resolution=RESOLUTION):
    ...
    # Create a PrettyMIDI object
    midi = pretty_midi.PrettyMIDI(initial_tempo=tempo,resolution=RESOLUTION)
